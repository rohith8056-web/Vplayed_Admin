...
- generic [active] [ref=e1]:
  ...
  - generic [ref=e606]:
    ...
    - alertdialog "Good, You have logged in successfully" [ref=e608]
```
...
> 155 |   const statusText = await adManagementPage.createdAdstatus(adName).innerText();
      |                                                                     ^ TypeError: adManagementPage.createdAdstatus(...).innerText is not a function
  156 | 
  157 |   if (statusText === 'Active') {
  158 |    await adManagementPage.adstatusactivatedeactivate(adName);
  159 |    await page.waitForTimeout(2000);
  160 |    await adManagementPage.adsstatuspopupyesbtn.click();
  161 |    await page.waitForTimeout(2000);
  162 |    await expect(adManagementPage.createdAdstatus(adName)).toHaveText('Inactive');
  163 |   } else {
  164 |    await adManagementPage.adstatusactivatedeactivate(adName);
  165 |    await page.waitForTimeout(2000);
  166 |    await adManagementPage.adsstatuspopupyesbtn.click();
  167 |    await page.waitForTimeout(2000);
  168 |    await expect(adManagementPage.createdAdstatus(adName)).toHaveText('Active');
  169 |   }
  170 | 
  171 |   });
  172 | 
