...
> 180 |    await expect(box.width).toBeCloseTo(160.796875, 1);
      |                            ^ Error: expect(received).toBeCloseTo(expected, precision)
  181 |    await expect(box.height).toBeCloseTo(32.796875, 1);
  182 |    await expect(adManagementPage.heading).toHaveText(admanagementdatafixture[0].assert1);
  183 |    await expect(adManagementPage.heading).toHaveCSS('font-family', 'Roboto, "Helvetica Neue", sans-serif');
  184 |    await expect(adManagementPage.heading).toHaveCSS('font-size', '20px');
  185 |    await expect(adManagementPage.heading).toHaveCSS('font-weight', '500');
  186 |    await expect(adManagementPage.heading).toHaveCSS('line-height', '32px');
  187 |    await expect(adManagementPage.heading).toHaveCSS('letter-spacing', '0.15px');
  188 |    await expect(adManagementPage.heading).toHaveCSS('color', 'rgb(33, 33, 33)');
  189 |    await expect(adManagementPage.heading).toHaveCSS('text-align', 'left');
  190 | 
  191 |    });
  192 | 
  193 |   
