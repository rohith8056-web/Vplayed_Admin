...
```
TypeError: adManagementPage.createdAdstatus is not a function
```
...
```ts
  ...
  136 |   await expect(await adManagementPage.createdAdstatusactive(adName)).toHaveText(admanagementdatafixture[5].assert2);
...
> 155 |   const statusLocator = await adManagementPage.createdAdstatus(adName); 
  ...
  164 |    await expect(await adManagementPage.createdAdstatusdeactive(adName)).toHaveText('Inactive');
  165 |   } else {
  ...
  170 |    await expect(await adManagementPage.createdAdstatusactive(adName)).toHaveText('Active');
  171 |   }
  172 | 
  173 |   });
  174 | 
