...
```
Error: toHaveCSS can be only used with Locator object, was called with Object { x: 255.203125, y: 95.1953125, width: 160.125, height: 24.1484375 }
```
...
```ts
  ...
  177 |    await expect(Heading).toBeVisible();
> 178 |    await expect(box).toHaveCSS('x', '255.203125px');
      |                      ^ Error: toHaveCSS can be only used with Locator object, was called with Object { x: 255.203125, y: 95.1953125, width: 160.125, height: 24.1484375 }
  179 | 
  180 |    console.log('Heading bounding box:', box.x);
  181 | 
  182 |    });
  183 | 
  184 |   
