...
> 186 |    await expect(adManagementPage.heading).toHaveCSS('line-height', '25.875px');
      |                                           ^ Error: expect(locator).toHaveCSS(expected) failed
  187 |    await expect(adManagementPage.heading).toHaveCSS('text-transform', 'capitalize');
  188 |    await expect(adManagementPage.heading).toHaveCSS('color', 'rgb(17, 18, 23)');
  189 |    await expect(adManagementPage.heading).toHaveCSS('align-items', 'center');
  190 | 
  191 |    });
  192 | 
  193 |   
