...
- generic [ref=e6]:
  - img "Vplayed" [ref=e9]
  - generic [ref=e10]:
    - heading "Forgot your password?" [level=1] [ref=e11]
    - paragraph [ref=e12]: Enter your email address below and we’ll send you verification code
    - generic [ref=e13]:
      - generic [ref=e14]:
        - generic [ref=e16]:
          - textbox [ref=e17]: admingsvplayed@gmail.com
          - generic [ref=e18]: Email ID
        - button [active] [ref=e20] [cursor=pointer]:
          - img [ref=e22]
      - paragraph [ref=e25]: Go back to Sign In
```
...
> 116 |     await expect(Forgetpassword.forgetpasswordpagesigninclick).tohaveurl('https://admin.vplayed.com/login');
      |                                                                ^ TypeError: expect(...).tohaveurl is not a function
  117 |     await expect(Forgetpassword.loginPageHeading).toContainText(logindatafixture.TC01.assert);
  118 |     await expect(signinPage.signInIntroText).toContainText(logindatafixture.TC02.assert);
  119 |     await expect(signinPage.forgotPasswordLink).toContainText(logindatafixture.TC03.assert);
  120 |     await expect(signinPage.emailInput).toContainText(logindatafixture.TC04.assert);
  121 |     await expect(signinPage.passwordPlaceholder).toContainText(logindatafixture.TC05.assert);
  122 | 
  123 | 
  124 |   });
  125 |   
